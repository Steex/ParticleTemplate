#ifndef _PSYS_STATIC_CURVE_INCLUDED_
#define _PSYS_STATIC_CURVE_INCLUDED_
...
namespace ParticlesStatic
{
    ...
    template<typename T>
    ParticlesStatic::Curve<T>::Curve(std::vector<T> _data, USize _table_size /*= 64*/)
    ...
    template<typename T>
    ParticlesStatic::Curve<T>::Curve(const Range<T>& _value_range, iCurve *_smooth_curve, USize _table_size /*= 64*/)
        : table_size  (_table_size)
        , fetch_table (_table_size)
    {
    ...
    template<typename T>
    ParticlesStatic::Curve<T>::Curve(iXml *_xml)
    ...
    template<typename T>
    T ParticlesStatic::Curve<T>::operator()(Float _arg)
    {
        if (_arg <= 0.0f)
        {
...
#endif // _PSYS_STATIC_CURVE_INCLUDED_

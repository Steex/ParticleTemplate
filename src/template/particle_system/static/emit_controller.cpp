...
ParticlesStatic::EmitController::EmitController(iXml *_xml)
    : active          (false)
    , looped          (false)
...
ParticlesStatic::EmitController::~EmitController()
{
}



void ParticlesStatic::EmitController::start()
{
    time = 0.0f;
    last_time = 0.0f;
    active = true;
...
void ParticlesStatic::EmitController::pause()
{
    active = false;
}
...
void ParticlesStatic::EmitController::resume()
{
    active = true;
}
...
USize ParticlesStatic::EmitController::process(Float _tick)
{
    if (!active)
    {
        return 0;

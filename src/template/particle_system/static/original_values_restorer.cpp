...
ParticlesStatic::OriginalValuesRestorer::OriginalValuesRestorer(ParamInfoHolder& _param_info)
    : value_info_list ()
{
...
ParticlesStatic::OriginalValuesRestorer::~OriginalValuesRestorer()
{
}



void ParticlesStatic::OriginalValuesRestorer::storeValues(Byte *_start, USize _stride, USize _count)
{
...
void ParticlesStatic::OriginalValuesRestorer::restoreValues(Byte *_start, USize _stride, USize _count)
{
    for (Byte *particle = _start, *last_particle = _start + _stride * (_count - 1); particle <= last_particle; particle += _stride)
    {
        FOREACH (ValueInfoList::const_iterator, value, value_info_list)
